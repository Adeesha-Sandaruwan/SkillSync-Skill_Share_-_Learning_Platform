...
const ShortsViewer = ({ posts, startIndex, onClose, onUpdatePost }) => {
    ...
    // --- FIX: FIND THE VIDEO FILE EXPLICITLY ---
    // Instead of grabbing index [0], we search for the .mp4/.webm file
    const getMediaUrl = (p) => {
        const urls = p.mediaUrls && p.mediaUrls.length > 0 ? p.mediaUrls : (p.imageUrl ? [p.imageUrl] : []);
        // Find first video
        const video = urls.find(url => {
            const u = url.toLowerCase();
            return u.endsWith('.mp4') || u.endsWith('.webm') || u.endsWith('.mov');
        });
        // Return video if found, otherwise return first item (fallback, though HomeFeed should prevent this)
        return video || urls[0];
    };
            ...
            <div className="relative w-full h-full md:max-w-[450px] bg-gray-900 flex items-center justify-center overflow-hidden shadow-2xl">
                ...
                <div className="absolute right-4 bottom-20 flex flex-col items-center gap-6 z-30">
                    ...
                    <div className="flex flex-col items-center gap-1">
                        <button onClick={() => handleReaction(myReaction || 'LIKE')} className={`w-12 h-12 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center active:scale-90 transition-transform ${myReaction ? 'text-red-500' : 'text-white'}`}>
                    ...
                    <div className="flex flex-col items-center gap-1">
                        <button onClick={() => setShowComments(true)} className="w-12 h-12 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center text-white active:scale-90 transition-transform">
                ...
                <div className="absolute left-4 bottom-6 right-20 z-30 text-white">
                    ...
                    <p className="text-sm font-medium opacity-90 leading-relaxed drop-shadow-md">{displayPost.description}</p>
                </div>

                {showComments && (
                    <div className="absolute inset-0 z-40 bg-black/60 flex flex-col justify-end animate-fade-in">
                        <div className="bg-white rounded-t-3xl h-[70%] w-full flex flex-col overflow-hidden animate-slide-up" onClick={e => e.stopPropagation()}>
