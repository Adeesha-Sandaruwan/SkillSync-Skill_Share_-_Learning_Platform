...
const Navbar = () => {
    ...
    const fetchUnreadCount = () => {
        if (user) {
            api.get(`/messages/unread/count?userId=${user.id}`)
                ...
                .catch(err => console.error("Badge fetch failed", err));
        }
    };
    ...
    useEffect(() => {
        ...
        // 2. Listen for the custom event fired by Chat.jsx when messages are read
        window.addEventListener('messages-read', fetchUnreadCount);

        // 3. Poll every 30 seconds to keep it synced in background
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-1.5 md:gap-4 flex-shrink-0">
                        ...
                        <Link to="/chat" className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 active:bg-indigo-100 active:scale-95 rounded-full transition-all relative group">
                            {unreadCount > 0 && (
                                <span className="absolute top-1 right-1 bg-red-500 text-white text-[9px] font-bold w-4 h-4 flex items-center justify-center rounded-full border-2 border-white animate-pulse">
...
const CompassIcon = ({ filled }) => (<svg className="w-6 h-6 transition-all" fill={filled ? "currentColor" : "none"} stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={filled ? "2" : "2"} d="M11 4a2 2 0 114 0v1a1 1 0 001 1h3a1 1 0 011 1v3a1 1 0 01-1 1h-1a2 2 0 100 4h1a1 1 0 01-1 1H7a1 1 0 01-1-1v-3a1 1 0 00-1-1H4a2 2 0 110-4h1a1 1 0 001-1V7a1 1 0 011-1h3a1 1 0 001-1V4z" /></svg>);
