                    ...
                    <div className="flex-1 text-center md:text-left mt-2 w-full">
                        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
                            ...
                            {isOwner ? (
                                ...
                                <button onClick={handleFollowToggle} disabled={loadingFollow} className={`px-8 py-2.5 rounded-xl font-bold shadow-lg transition-all active:scale-95 ${isFollowing ? 'bg-white border-2 border-slate-200 text-slate-600' : 'bg-indigo-600 text-white hover:bg-indigo-700'}`}>
                        ...
                        <div className="grid grid-cols-3 md:grid-cols-6 gap-2 border-t border-slate-100 pt-6">
                            {[
                                ...
                                { label: 'Plans', val: stats.totalPlans }, { label: 'Steps Done', val: stats.totalStepsCompleted, highlight: true }
                            ].map((stat, idx) => (
                                <div key={idx} className="text-center p-2 rounded-lg hover:bg-slate-50 transition-colors">
                                    <div className={`text-xl md:text-2xl font-black ${stat.highlight ? 'text-indigo-600' : 'text-slate-800'}`}>{stat.val}</div>
                            ...
                            {isOwner && (
                                <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
                                    <form onSubmit={handlePostUpdate} className="flex flex-col gap-4">
                                        <textarea placeholder="What did you learn today?" className="w-full bg-slate-50 border-transparent rounded-xl p-4 focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all resize-none" rows="2" value={newUpdate} onChange={e => setNewUpdate(e.target.value)} />
